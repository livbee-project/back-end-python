...
class ChatRoom(Base):
    ...
    brand_user_id = Column(
        String, ForeignKey("users.id", ondelete="CASCADE"), nullable=False, index=True
    )
    showhost_user_id = Column(
        String, ForeignKey("users.id", ondelete="CASCADE"), nullable=False, index=True
    )
    ...
    brand_user = relationship(
        "User", foreign_keys=[brand_user_id], back_populates="brand_chat_rooms"
    )
    showhost_user = relationship(
        "User", foreign_keys=[showhost_user_id], back_populates="showhost_chat_rooms"
    )
    application = relationship(
        "Application", back_populates="chat_room", foreign_keys=[application_id]
    )
...
class ChatMessage(Base):
    ...
    id = Column(String, primary_key=True, default=_uuid)
    room_id = Column(
        String, ForeignKey("chat_rooms.id", ondelete="CASCADE"), nullable=False, index=True
    )
    sender_id = Column(
