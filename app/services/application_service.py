...
def create_application(
    ...
    # 캠페인 확인
    campaign = get_or_404(
        db, Campaign, lambda q: q.filter(Campaign.id == campaign_id), error_key="NOT_FOUND"
    )
    ...
    existing = (
        ...
        .filter(Application.campaign_id == campaign_id, Application.user_id == user_id)
        .first()
    )
...
def get_application_by_campaign_and_user(
    db: Session, campaign_id: str, user_id: str
) -> Optional[Application]:
    """
    캠페인과 사용자로 지원서 조회 (campaign, user, portfolio, model eager load로 N+1 방지)
    ...
    return (
        db.query(Application)
        .options(
            joinedload(Application.campaign),
            joinedload(Application.user),
            joinedload(Application.portfolio),
            joinedload(Application.model),
        )
        .filter(Application.campaign_id == campaign_id, Application.user_id == user_id)
        .first()
    )
...
def get_applications_by_campaign(
    db: Session, campaign_id: str, user_id: str, user_role: str
) -> list[Application]:
    """
...
def update_application_status(
    db: Session, application_id: str, new_status: ApplicationStatus, user_id: str, user_role: str
) -> Application:
    """
    지원서 상태 변경
