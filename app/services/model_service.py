...
def get_model_by_id(
    db: Session, model_id: str, user_id: Optional[str] = None, user_role: Optional[str] = None
) -> Model:
    """
    모델 ID로 조회 (user 관계 eager load로 N+1 방지)
    ...
    model = get_or_404(
        db,
        Model,
        lambda q: q.options(joinedload(Model.user)).filter(Model.id == model_id),
        error_key="NOT_FOUND",
    )
    ...
    if user_id:
        require_ownership_or_admin(
            model, user_id, user_role, owner_field="user_id", error_key="MODEL_FORBIDDEN_EDIT"
        )
...
def get_public_models(db: Session, page: int = 1, limit: int = 20) -> tuple[list[Model], int]:
    """
    공개된 모델 목록 조회
    ...
    query = db.query(Model).filter(Model.public_scope == "전체공개", Model.status == "published")
...
def get_published_models(db: Session, page: int = 1, limit: int = 10) -> tuple[list[Model], int]:
    """
    ...
    query = db.query(Model).filter(Model.status == "published")
...
def check_user_has_model(db: Session, user_id: str) -> bool:
    """
    사용자가 모델을 가지고 있는지 확인
...
def create_model(db: Session, user_id: str, model_data: Dict[str, Any]) -> Model:
    """
    모델 생성
...
def delete_model(db: Session, model_id: str, user_id: str, user_role: Optional[str] = None) -> None:
    """
    모델 삭제
