...
def test_ensure_chat_room_create_new(
    ...
    # 참가자 확인
    participants = (
        db_session.query(ChatParticipant).filter(ChatParticipant.room_id == room.id).all()
    )
...
def test_ensure_chat_room_existing(
    ...
    # 참가자는 중복 생성되지 않아야 함
    participants = (
        db_session.query(ChatParticipant).filter(ChatParticipant.room_id == room1.id).all()
    )
