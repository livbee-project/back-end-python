...
def test_news(db_session, test_user):
    """테스트용 뉴스 생성"""
    news = News(
        id=str(uuid.uuid4()), title="Test News", content="Test content", created_by=test_user.id
    )
    db_session.add(news)
    db_session.flush()
    return news
...
def test_get_news_list_pagination(db_session, test_user):
    ...
    for i in range(5):
        news = News(
            id=str(uuid.uuid4()), title=f"News {i}", content=f"Content {i}", created_by=test_user.id
        )
        db_session.add(news)
    db_session.flush()
...
def test_update_news_success(db_session, test_news):
    """뉴스 수정 성공 테스트"""
    updated = update_news(
        db_session, news_id=test_news.id, title="Updated Title", content="Updated content"
    )
...
def test_update_news_partial(db_session, test_news):
    ...
    updated = update_news(db_session, news_id=test_news.id, title="Updated Title")
