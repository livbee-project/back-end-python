...
def upgrade() -> None:
    ...
    # models 테이블: 이미 있으면 스킵 (DuplicateTable 방지)
    r = conn.execute(
        sa.text(
    ...
    # proposals: target_model_id 등 — idempotent (이미 있으면 스킵)
    conn.execute(sa.text("ALTER TABLE proposals ADD COLUMN IF NOT EXISTS target_model_id VARCHAR"))
    conn.execute(sa.text("CREATE INDEX IF NOT EXISTS ix_proposals_target_model_id ON proposals (target_model_id)"))
    conn.execute(
        sa.text(
            "DO $$ BEGIN "
            "ALTER TABLE proposals ADD CONSTRAINT fk_proposals_target_model_id "
    ...
    # applications: portfolio_id, model_id — idempotent
    conn.execute(sa.text("ALTER TABLE applications ADD COLUMN IF NOT EXISTS portfolio_id VARCHAR"))
    conn.execute(sa.text("ALTER TABLE applications ADD COLUMN IF NOT EXISTS model_id VARCHAR"))
    conn.execute(sa.text("CREATE INDEX IF NOT EXISTS ix_applications_portfolio_id ON applications (portfolio_id)"))
    conn.execute(sa.text("CREATE INDEX IF NOT EXISTS ix_applications_model_id ON applications (model_id)"))
    conn.execute(
        sa.text(
            "DO $$ BEGIN "
            "ALTER TABLE applications ADD CONSTRAINT fk_applications_portfolio_id "
    ...
    conn.execute(
        sa.text(
            "DO $$ BEGIN "
            "ALTER TABLE applications ADD CONSTRAINT fk_applications_model_id "
